...
export const getAllTransactionsByPeriodValidation = {
  query: z.object({
    startMonth: z.coerce.number({ error: "O mês de início é obrigatório" }).min(1).max(12),
    startYear: z.coerce.number({ error: "O ano de início é obrigatório" }).min(2000).max(2100),
    endMonth: z.coerce.number({ error: "O mês de término é obrigatório" }).min(1).max(12),
    endYear: z.coerce.number({ error: "O ano de término é obrigatório" }).min(2000).max(2100),
  }).refine(
...
export type GetAllTransactionsByPeriodDTO = z.infer<typeof getAllTransactionsByPeriodValidation.query>